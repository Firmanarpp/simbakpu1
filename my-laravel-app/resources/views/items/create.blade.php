{{--
/**
 * Item Create Page - Halaman Tambah Barang Baru
 * 
 * File: resources/views/items/create.blade.php
 * Deskripsi: Form untuk menambahkan barang baru ke dalam sistem
 * Fitur: Form validation, auto QR generation, room selection, responsive design
 * 
 * Komponen yang ditampilkan:
 * - Page header dengan judul dan breadcrumb
 * - Form input untuk detail barang (Brand, Type, Description, dll)
                ...
                <div class="card-body p-3">
                    @if(session('info'))
                        <div class="alert alert-info alert-dismissible fade show" role="alert">
                            ...
                            <span class="d-none d-sm-inline">{{ session('info') }}</span>
                            <span class="d-sm-none">{{ Str::limit(session('info'), 40) }}</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (qrCodeInput.value.trim() === '') {
            e.preventDefault();
            alert('Silakan masukkan atau generate kode QR terlebih dahulu!');
            qrCodeInput.focus();
            return;
        }
    });
