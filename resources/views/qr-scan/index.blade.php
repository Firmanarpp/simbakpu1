{{--
/**
 * QR Code Scanner Page - Halaman Scan QR Code
 * 
 * File: resources/views/qr-scan/index.blade.php
 * Deskripsi: Halaman untuk scan QR code barang menggunakan kamera
 * Fitur: Camera integration, QR detection, automatic item lookup, responsive design
 * 
 * Komponen yang ditampilkan:
 * - Page header dengan instruksi scan
 * - Camera preview area dengan overlay
 * - Camera controls (start/stop, switch camera)
 * - Scan result display
 * - Item information ketika QR terdeteksi
 * - Error handling untuk permission dan detection
 * - Manual input fallback
 * 
 * Dependencies:
 * - HTML5 QR Code Scanner library
 * - Camera API permissions
 * - AJAX untuk item lookup
 * 
...
@section('title', 'Scan QR Code - Sistem Manajemen Barang Kantor')
            ...
            <div class="card">
                ...
                <div class="card-body p-3">
                    ...
                    <div class="border-top pt-4">
                        ...
                        <form action="{{ route('qr-scan.scan') }}" method="POST" id="manual-form">
                            @csrf
                            <div class="row g-2">
    ...
    function submitQRCode(qrCode) {
        ...
        .then(data => {
            if (data.success) {
                ...
                `;
                setTimeout(() => {
                    window.location.href = `{{ route('items.create') }}?qr_code=${encodeURIComponent(qrCode)}`;
                }, 1500);
            }
        })
        .catch(error => {
            ...
            if (form && input) {
                ...
                window.location.href = `{{ route('qr-scan.scan') }}?qr_code=${encodeURIComponent(qrCode)}`;
            }
        });
    }
